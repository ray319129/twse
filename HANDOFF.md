...
- **驗證**:extract_json 對 純JSON/```json 圍欄/前後夾雜文字/非JSON 皆正確;news `analyze_news→compute` 與 catalyst `classify_catalysts`(含圍欄回應)mock 測試皆過。**⚠️ 尚未在真 Vercel 跑過修正後版本**(需 push 後部署,再查一檔看新聞面向是否出分)。
