...
## 62. 8/14 盤中健檢報告對帳 —— 找出 10 個問題,全部修完(2026-08-14)
...
### ⑨⑩ 續修(同日稍後,commit 見 git log)

**⑨ 盤中未收盤 K 棒 —— 標記而非丟棄**
新增 [`engine.is_partial_last_bar()`](scripts/health/engine.py):最後一根 K 棒是「今天且台北時間
早於 13:35」就判定為盤中未完成棒,結果放進 `ctx["partial_last_bar"]`,批次/即時兩條路徑共用。
**刻意不丟棄那根 K 棒** —— 丟掉盤中健檢就完全看不到今天,比失真更糟。

只有「被半天成交量直接扭曲」的兩個指標改用**上一根完整 K 棒**(等於「截至昨收的均量」):
- `technical_engine` 的量能結構(`vol_ma5`/`vol_ma20` 取 `iloc[-2]`)
- `chip_engine` 的日均成交金額(收盤價 × 20日均量,整列取 `iloc[-2]`)—— 這個數字還會往下餵
  `swing_scores` 的流動性百分位,不修會連短線評估一起帶偏

價格類指標(均線/RSI/KD/MACD/ATR/支撐壓力)用即時價本來就是對的,不動;改在 asof 標
「(盤中未收盤)」+ engine notes 加一行警語(前端 `hm-note` 本來就會渲染 notes,不必改前端)。

用真實 8/13 資料 + 依報告反推的 8/14 盤中量合成 partial bar 驗證:

| | 量能結構 舊→新 | 日均成交額(百萬) 舊→新 |
|---|---|---|
| 2303 | 0.66 → **0.72** | 23,125.0 → **24,713.4** |
| 2337 | 1.31 → **1.42** | 11,273.9 → **11,210.6** |
| 2344 | 1.37 → **1.39** | 25,728.7 → **24,291.7** |
| 2449 | 1.03 → **1.01** | 5,314.1 → **5,756.1** |

新值與對帳當初用完整 K 棒(到 8/13)重算的數字**完全一致**,閉環驗證通過。
(2449 的 swing 分數 92/93/70 沒變 —— 百分位曲線在那個量級很平,屬正常。)

**⑩ 新聞視窗重複 —— 先更正機制**
⚠️ **對帳報告當時的說法是錯的**:我寫「`fetch_news(stock_id, name, 60)` 來源只有 60 天」,
但那個 `60` 是 **`limit`(則數上限),不是天數**。實測 `fetch_news('2449', ..., limit=100)`
回得到 **2026-02-09** 的新聞,RSS 根本沒有 3 週的天花板。

真正的機制是 **`config` 的 `news.ai_max_items: 30`**:只有最新 30 則會送 AI 逐則分類
(純呼叫無結構化輸出保護,太多則 JSON 會超 max_tokens 被截斷)。熱門股那最新 ~30 則
**全部落在 30 天內** → 90 天視窗一則都沒多,卻仍佔 0.2 權重,同一批新聞被算兩次。
所以「把 fetch_news 天數拉到 90」是無效的修法,拉高 `ai_max_items` 又會踩回截斷的老坑。

改法:`news_engine.compute` 先算三個視窗的 stats,**則數與較短視窗相同就判定為「無額外資料」**,
輸出 `missing_metric(not_applicable)` 並說明原因,不重複計分 —— `score` 本來就除以
`weighted_score_w`,跳過的權重會自動讓回較短視窗。另加一條 `news_coverage_span` 指標
把「本次分析的新聞回溯範圍」直接寫出來,不讓讀者以為真的涵蓋 90 天。

實測(用 2449 8/14 報告的 15 則重建):近7天 0.23 / 13 則、近30天 0.20 / 15 則、
**近90天 → 不適用**(附「與近30天完全相同(15 則)…權重已讓回近30天視窗」),
回溯範圍顯示 `2026-07-26 ~ 2026-08-13`。

**融券回補加絕對量下限**
`_SHORT_COVER_MIN_LOTS = 500`,比例 ≥5% **且** 減少 ≥500 張才算回補中;比例達標但量太小
顯示「量太小不算回補」(rating=neutral)並在 formula 註明是雜訊。
實測:2449(227→209 張,差 18 張)→ **量太小不算回補**;2344(5,415→4,552 張,差 863 張)→ **回補中**。

⚠️ **又踩到同一類坑**:`cover` 在「5日前融券餘額為 0」時是 None,f-string 直接
`cover * 100` → `TypeError`。跟 §62 的 `kind_txt` dict 是同一種錯(**急著格式化可能為 None 的值**),
一樣是全市場掃描才抓到。**寫 formula 字串時,凡是來自除法/條件賦值的變數都要先確認非 None。**
...
### 驗證
- 全市場 **308 檔 × `partial_last_bar` True/False 兩種**跑完**七個** Engine + `swing_scores`,**例外 0**
- 四檔逐項對照修正前後(見上方各節表格);⑨ 的新值與對帳當初用完整 K 棒重算的數字完全一致
- 邊界:`swing_scores(None, None, ...)`、ATR=0 / 成交額=0、空 revenue、
  `_stale_month_index(None)`、`is_partial_last_bar(None/空DF)`、融券 5 日前餘額為 0
- 前端 `node --check` OK(engine `notes` 本來就會渲染成 `hm-note`,⑨ 的警語不必改前端)

### 給未來的自己:這一輪重複踩到的兩件事
1. **急著格式化可能為 None 的值** —— dict 查表會先算完所有 value(§62 的 `kind_txt`)、
   除法結果可能是 None(⑨⑩ 那輪的 `cover`)。兩次都是**四檔測試沒踩到、全市場掃描才抓到**。
2. **對帳結論要區分「觀察到的現象」與「推斷的機制」** —— ⑩ 我把 `fetch_news(..., 60)` 的
   `limit` 誤讀成天數,現象(30/90 天視窗完全相同)是對的,機制講錯了,修法也就跟著錯
   (拉天數無效,真正的瓶頸是 `ai_max_items`)。**下結論前先讀函式簽名。**

---

## 63. 訂閱到期 → 盤中掃描整週空轉:改成降級而非停擺(2026-08-25)

### 起因
使用者要求「沒訂閱跟訂閱時都要能正常使用」。查下去發現 **FinMind Sponsor 於 2026-08-17 到期**
(`docs/data.json` 的 `realtime.sponsor` 寫得很清楚:`{active:false, level_title:"Free",
expires:"2026-08-17", days_left:-7}`),而盤中掃描從 **08-18 起整整一週沒有跑過任何一輪**。

`gh run view` 日誌的關鍵三行:
```
非 Sponsor / 訂閱到期 —— 沒有全市場即時快照,盤中掃描跳過。
停止盯盤:no_sponsor
盯盤結束:輪詢 2 次、觸發 0 筆、錯誤 0 次
```
`scan()` 直接 `return no_sponsor`,watch 迴圈收到就 `break` → 08:20 啟動的 job 睡到 09:00、
掃兩輪就結束(run 時長 41 分鐘,其中 40 分鐘在睡)。`docs/alerts.json` / `docs/pulse.json`
都凍在 08-17,分點資料也是。

### 沒訂閱時的實際健康度(查完的完整結論)
| 功能 | 狀態 |
|---|---|
| 盤後選股 / 台帳 / 網頁主資料 | ✅ 正常(`data.json` 每日更新,不碰即時層) |
| 盤前快報 | ✅ 正常(本來就走 MIS 試撮) |
| 個股健檢 | ✅ 正常(不依賴 Sponsor) |
| 報價卡 / 持倉頁 | ✅ 正常(`get_quotes` 三級降級本來就有) |
| 前端「資料源」標示 | ✅ 誠實顯示「非即時 — 已降級」,過期訊號會隱藏(`d.date >= today`) |
| 鐵則一(即時不進選股) | ✅ `check_realtime_isolation` 通過 |
| **盤中掃描 / 訊號** | ❌ **完全停擺** ← 本次修 |
| 分點(Sponsor 級) | ⚠️ 抓不到是事實,但日誌**講錯原因** ← 本次修 |
| 全市場快照存檔 | ⚠️ 略過(設計如此,訊息已誠實) |

### 改法:縮小宇宙、保留規則(不是關掉功能)
MIS 那層一直是活的,只是「逐檔查、50 檔一批、易被 ban」,掃不動全市場 2852 檔。
所以**不掃全市場,改掃使用者真正在看的那些**:當日核心 + 觀察 + 自選池
(實測 core 7 + watch 20 + watchlist 8 = 35 檔 = 剛好一批 MIS,一輪 1~2 秒)。

新增 [`quotes.fetch_snapshot_degraded()`](scripts/quotes.py):把 MIS 的欄位補成
**與 `fetch_snapshot_all` 完全相同的 schema**,所以 `_signals()` 的規則、門檻、去重
**一行都沒改**。兩個 Sponsor 專屬欄位改用估計值:

- **`average_price`(均價/VWAP)**:MIS 沒這欄。改用**自己的輪詢**累積 ——
  Δ量 = 本輪累積量 − 上輪累積量,`VWAP ≈ Σ(價×Δ量) ÷ ΣΔ量`,從 09:00 第一輪開始累。
  第一輪沒有 Δ量 → 回 `None`,該輪「站上均價」自然不成立(寧可漏不可錯)。
  單元測試:四輪 (100,1000)(102,3000)(101,6000)(105,10000) → 估出 103.0000,
  手算 `(102×2000+101×3000+105×4000)/9000 = 103.0000`,**完全吻合**;跨日自動重置。
- **`volume_ratio`(量比)**:Sponsor 版是「vs 昨日同時段」。改用
  `當日累積量 ÷ (20日均量 × 已開盤時間佔比)`,20日均量由 `levels.avg_turnover ÷ prev_close` 還原。
  語意相近但**不是同一個定義**,所以降級訊號一律標記。

### 三個必要的配套(每一個都是實測踩出來的)
1. **上櫃股會被靜默丟掉** —— MIS 頻道分 `tse_`/`otc_`,裸字串一律當上市查就少那幾檔、
   **且回應裡完全沒有警告**。實測 35 檔宇宙裡 8 檔上櫃(含核心股 6290、4707)全部消失,
   只拿回 27 檔。新增 `quotes._market_map()` 補市場別 → 35/35。
2. **非交易時段必須擋** —— MIS 在盤後回的是**上一交易日收盤**,但降級快照的 `date`
   蓋的是「現在」→ `scan()` 原本那道「時間戳 ≠ 今天就跳過」的保護**失效**,
   盤後跑一次就會拿昨天的收盤當今天的即時價觸發訊號。新增 `quotes.in_trading_session()`,
   scan 的降級分支先擋(`reason="outside_session"`),量比也只在時段內給值。
3. **輪詢節奏要放慢** —— watch 預設 10 秒是照 FinMind 上游頻率調的;打在 MIS 這種非官方
   端點上是 6 次/分鐘,風險高又無意義(MIS 本來就不是 10 秒級)。降級時
   `DEGRADED_MIN_INTERVAL = 30` 秒。

### 其他修正
- **`vol_base` 不能用小樣本自我參照**:量比門檻的基準原本是「當日全市場量比中位數」,
  降級時樣本只有那 30~50 檔選股 —— 它們本來就偏強勢/偏爆量,拿它們的中位數當
  「市場中位數」會把門檻推高、反而漏訊號。降級時誠實退回固定基準 1.0。
- **鐵則二(降級要看得見)**:每筆 alert 帶 `degraded`,Discord footer 加
  「⚠ 降級模式:量比/均價為估計值,僅掃自選+選股」,網頁 `alertsBox` 加一塊警示說明。
- **分點日誌講錯原因**([branch_chips.py](scripts/branch_chips.py)):資料抓不到時一律印
  「可能未到 21:00 發布時間或非交易日」—— 但 2026-08-24 那次跑在**台北 22:23**(早過 21:00)
  還是印這句,結果資料從 08-17 靜靜停更一週都看不出原因。改成先問 `sponsor_status()`,
  是訂閱問題就明講「Sponsor 級資料集,沒有訂閱就抓不到 —— 不是時間或交易日的問題」。
- **watch 迴圈的 `no_sponsor` 停止條件移除**,只剩 `no_levels` / `no_universe`
  (這兩個再輪一百次也不會自己好)。

⚠️ **踩到的坑**:`last_degraded` 一開始寫成直接讀 `r.get("degraded")`,但 `r` 只在 `try`
內賦值 —— 第一輪就拋例外的話,後面的節奏判斷會 `NameError` 把整個盯盤打掉(比原本的
bug 還嚴重)。改成迴圈外先初始化的旗標。

### 驗證
- **降級路徑**:35/35 檔取得(含上櫃)、量比公式獨立手算對過
  (2030:8/24 成交 6,539 張 ÷ 20日均量 2,560 張 = 2.55 倍,÷時段佔比 0.5 = 5.09 ✓)、
  MIS 成交量單位確認是「張」(MIS 6,508 vs parquet 6,539 張)
- **規則層**:用降級 schema 造「量增突破」與「回檔買點」兩種情境,**都能正常觸發**
- **有訂閱路徑沒被改壞**:mock `sponsor_status active` + 300 檔全市場快照 →
  `degraded=False`、`checked=300`,走的仍是全市場那條
- 非交易時段跑 `scan()` → `outside_session`,不觸發任何訊號
- `check_realtime_isolation` 通過、`compileall` 通過、`node --check` 通過
- 健檢七個 Engine × 365 檔回歸,例外 0

### 待辦(未做,需裁示)
- **本機 repo 曾落後遠端 11 天** —— 這次是我 `git pull` 才發現。不影響線上,但本機跑
  任何分析前要先 pull,否則會用到舊資料算出舊結論。
- `docs/premarket.json` 最外層 `date` 是「被盯的那組選股的日期」而非今天
  (8/24 檔案裡是 `2026-08-21`)。**前端沒用到這個欄位**(只用 `preopen`/`orb` 各自的
  `generated_at`),所以目前無害,但欄位名容易誤導。
- `push-with-retry.sh` 在 8/24 的日誌裡出現 `cannot pull with rebase: You have unstaged
  changes` → `fatal: no rebase in progress`。那次最後 push 成功,但這條路徑跟
  2026-07-22 盤中卡死是同一類(rebase 前沒清乾淨工作區),值得補強。
- `scripts/check_realtime_isolation.py` 在 Windows 主控台會因 `✓` 撞 cp950 而
  `UnicodeEncodeError`(CI 的 Linux 沒事)。跑之前加 `PYTHONIOENCODING=utf-8` 即可。

---

## 64. 當沖層上線 + 本專案第一組自動化測試(2026-09-08)

### 先報告:§63 的降級修正在生產環境成功了

| 期間 | 訊號/日 | 模式 |
|---|---|---|
| 8/11–8/17(有訂閱) | 40–61 筆 | 全市場 877 檔 |
| **8/18–8/24** | **0 筆** | ← 壞掉的那週 |
| 8/25–9/08(修正後) | **3–15 筆** | `degraded:True`,35 檔小宇宙 |

9/08 盤中一路跑到 13:35。**但本機又落後遠端 985 個 commit** —— 這已是第二次,
任何分析前務必先 `git pull`。

### 當沖層架構(`scripts/daytrade/`,與既有系統完全隔離)

使用者要求「當沖功能要獨立出來測試」,所以照鐵則一的精神做:
`check_realtime_isolation.py` 的 `REALTIME` 已加入 `daytrade`,
**選股/回測層不得 import 它**;自己的 config、資料夾、Discord 卡片、台帳;
`config/daytrade.yaml` 的 `enabled: false` 就整層停用。

| 模組 | 職責 |
|---|---|
| `cost.py` | tick 級距 + 來回成本計算(**整層的地基**) |
| `rules.py` | 六道法規閘門(可當沖/暫停先賣後買/處置/注意/借券費) |
| `universe.py` | 標的池:閘門 × 成本 × 使用者偏好 → 多空兩欄 |
| `signals.py` | 水位穿越偵測 + 訊號分級 |
| `risk.py` | 停損穿越 + **強制回補提醒** |
| `ledger.py` | 當沖台帳(含未推播對照組) |
| `engine.py` | orchestrator + Discord + CLI |

### 關鍵發現一:價格區間不是偏好問題,是成本問題

使用者要「自選價格範圍」。算完 tick 幾何後發現這根本是成本問題:

掃單來回成本 = 手續稅 0.321% + 進出各跨 1 檔價差

| 股價 | tick | 1檔% | 來回成本 | |
|---|---|---|---|---|
| 99 | 0.10 | 0.101% | **0.523%** | ★ 最便宜 |
| **105** | **0.50** | **0.476%** | **1.273%** | ✗ 最貴 |
| 499 | 0.50 | 0.100% | 0.521% | ★ |
| 1200 | 5.00 | 0.417% | 1.154% | ✗ |

**同一套策略在 105 元的股票上成本是 99 元的 2.4 倍。**
規律:tick 換檔的**正上方最貴、正下方最便宜**。危險帶 100~130 / 10~16 / 500~560 / 1000~1300。

所以 `universe.py` 不是給一個空白輸入框,而是每檔都附成本評級,
預設把 `expensive` 排除 —— 讓「自由選擇」不等於「自己踩坑」。

### 關鍵發現二:第一版排序選出一整排金融股

第一版 rank 用流動性 0.4 主導,結果前八名是兆豐金、華南金、第一金、台新新光金 ——
成交額大、成本低,但**一天走不到 1%**,對當沖毫無意義。
根因是我用 `high20/ma20` 距離當 ATR 代理,那量到的是「距月線多遠」不是「一天會走多少」。

改法:
1. `real_atr_pct()` 用價格序列算真 ATR(只對通過初篩的約 200 檔算,盤前批次,不打 API)
2. **波動改成硬條件**,不是軟權重
3. 新增 `edge_ratio = ATR% ÷ 來回成本%`(一天的波動能覆蓋幾次來回成本),**當排序主軸**
4. 買不起一張(額度 25 萬)就不進池 —— 顯示「可買 0 張」對使用者沒意義

修正後前八名 ATR 4~9%,而且 5425 台半(借券費 3%)正確地只出現在多方、被空方閘門擋掉。

### 訊號分級:本層的價值在「拒絕」

既有盤中掃描每日觸發 9~15 筆,而來回成本門檻 0.52% ——
**全做必虧**(15 × 0.52% = 7.8%)。所以 `ranking.push_top_n: 3`,
多空**各自**取前 3(不然強勢盤會讓多方把空方名額吃光),
其餘只進網頁與台帳當**對照組** —— `ledger.summarise()` 分開算 pushed vs not_pushed,
兩者的差距就是分級本身的價值。

### 資料源(全部免費免金鑰,2026-09-08 實測)

TWSE `TWTB4U`(含 `Suspension` 欄)/`announcement/punish`/`notice`;
TPEx `tpex_securities`/`tpex_disposal_information`/`tpex_trading_warning_information`/
`tpex_intraday_fee`(券差借券費率,實測 0.1%~3%)。

⚠️ **`Suspension="Y"` 是「暫停先賣後買」不是「不能當沖」** —— 名單裡有台泥、瑞昱
這種大型股,原因是除權息停止過戶。**只擋空方,不擋多方**;
當成「不能當沖」會少掉一堆可做標的。

### ⚠️ 踩到的坑(四個,都值得記)

1. **TPEx 憑證 + Python 3.13**:櫃買憑證缺 Subject Key Identifier,而
   **Python 3.13 起 `ssl.create_default_context()` 預設開 `VERIFY_X509_STRICT`**
   → 本機 3.14 全部上櫃 feed 掛掉(1233 檔 vs 修好後 2079 檔),curl 同機器 HTTP 200。
   CI 現在跑 3.11 看不出來,但**CI 一升到 3.13+,所有上櫃股會 fail closed 被靜默排除**。
   已加 `_RelaxedTLSAdapter`:只關 X509 擴充嚴格檢查,**憑證鏈與主機名驗證都保留**,
   且只掛在 tpex 這個 host。

2. **`is_etf` 判錯**:第一版寫 `len(sid) >= 5`,剛好把 **0050 / 0056** 這兩支最大的
   ETF 排除掉(它們是 4 碼)→ tick 會算錯 5 倍。**單元測試抓到的。**

3. **`(x or 99)` 的 falsy-zero**:冷卻判斷寫成 `(_mins(lp, hhmm) or 99) < cooldown`,
   同一分鐘內 `_mins` 回 **0**,而 `0 or 99` = 99 → 冷卻整個失效。**整合測試抓到的。**

4. **強制回補會倒退送**:送出 13:15 後,下一輪發現 13:00「還沒送過」又送一次比較不急的
   提醒。語意應該是「已提醒到這個急迫度」,所以送 X 時要把所有 ≤X 的一併標記。
   **整合測試抓到的。**

坑 2/3/4 的共同點:**手動測那幾檔都不會踩到,是測試抓出來的**。這正是補測試的理由。

### 本專案第一組自動化測試(`tests/`)

15,144 行 Python 原本 **0 個測試**。這次補了 6 個測試檔、約 60 條斷言,
全部針對純函數(成本/tick、法規閘門、訊號分級、風控方向、台帳、端到端整合),
不打網路、不寫 repo。跑法:`python tests/run_all.py`。
已接進 `daily.yml` 與 `daytrade.yml`,測試沒過就不跑批次。

**不追求覆蓋率**,只擋「四檔手動測不出、全市場掃描才現形」那類錯 —— 這種 bug
本專案已經踩過三次(§62 的 `kind_txt` dict eager eval、§63 的 `cover` 為 None、本次的三個)。

### 使用者偏好與額度(方案 B)

`config/daytrade_prefs.json`,走與 `api/watchlist.py` 同一套 GitHub Contents API + secret。
**額度預設 25 萬**,依使用者 2026-09-08 裁示採方案 B(額度上傳 repo,讓推播卡能直接算
可買張數)。⚠️ 這放寬了 2026-07-19「不上傳成本損益」的界線,但**只放寬到「額度上限」
這一個數字** —— 實際持倉張數與成本仍只留 localStorage。

### 排程

獨立的 `.github/workflows/daytrade.yml`:08:30 建池、09:00 盯盤到 13:35。
**刻意不併進 `intraday.yml`** —— 當沖層還在驗證階段,它掛掉不該連累已經穩定的盤中掃描。

### 待辦

- **MIS 盤中真實延遲還沒量到**(建置時已收盤)。這直接決定盯盤輪詢間隔該設多少;
  若延遲 >60 秒,第 2 層(水位穿越)的價值要重新評估,重心應移到第 1/3/4 層。
- 網頁的當沖分頁還沒做(`docs/daytrade.json` 已產出,前端未接)。
- 部位標記 UI 還沒做 —— 目前 `data/daytrade/positions-*.json` 要手動建,
  強制回補提醒才有東西可提醒。計畫復用既有 `my_marks` 的雲端同步機制。
- 觀察一個月後看 `ledger.summarise()` 的 pushed vs not_pushed,再決定門檻怎麼調。

---

## 65. 當沖層補完三項未完成(2026-09-08 續)

§64 收尾時留了三個未做,這一輪全部補上。

### ① MIS 真實延遲量測(`scripts/daytrade/latency.py`)

**為什麼非量不可**:第 2 層(價位穿越推播)的價值完全取決於延遲。若 MIS 落後 >60 秒,
「剛穿過 105.2」到手時價格早跑掉,那層就該降級成純記錄,重心移到標的池/風控/台帳
(那三層不受延遲影響)。**這件事不能用猜的。**

MIS 回傳的欄位剛好夠直接量測(不是推估):
- `t` 最後成交時間、`tlong` epoch ms、`queryTime.sysTime` 交易所主機時間
- **資料落後 = sysTime − t**

⚠️ 但 `t` 是「最後成交時間」,冷門股沒成交也會拉高 → 一律取多檔中位數,
並回報 P25/P75/P90 分布與「最佳檔中位延遲」,讓人分得出是系統延遲還是個股沒量。
另外量 `tlong` 多久變一次 = 上游實際更新頻率(輪詢比它密只會拿到同一份資料)。

`_verdict()` 把數字翻成結論:≤15s good(可用)/ ≤60s marginal(只適合水位穿越,
不要追動能)/ >60s poor(**第 2 層價值有限,建議降級成純記錄**)。

**只有交易時段量得到** —— 盤後 MIS 回上一交易日收盤,`t` 永遠 13:30:00,量出來是幾小時。
所以做成盯盤迴圈開盤後自動量一次(90 秒),寫進 `docs/daytrade_latency.json`,
結論直接顯示在網頁上。本機實測非交易時段會正確回 `{"skipped": ...}`。

**明天(9/09)開盤後就會有第一筆數據** —— 看到結果再決定第 2 層要不要調整。

### ② 網頁當沖分頁(`docs/index.html`)

新增「當沖」分頁,讀 `docs/daytrade.json` + `docs/daytrade_latency.json`,
與既有選股資料完全分開。刻意把**成本與「還沒驗證」講在最前面**,
不做成看起來很篤定的推薦清單:

- 頂部黃框:每筆來回成本門檻 0.52%、一天 3 筆要先賺 1.56% 才打平、
  只推多空各前 3、**請等台帳累積一個月再判斷有沒有用**
- 延遲實測框(有數據時顯示中位/P25/P75 與結論,顏色隨 good/marginal/poor)
- 篩選過程全攤開(掃 877 檔 → 各道閘門各擋掉幾檔)
- 多空兩欄,每檔顯示成本評級、tick、需賺幾檔打平、借券費、閘門狀態、空間倍數、可買張數

**瀏覽器實測驗證**:分頁正確渲染,台半(借券費 3%)與華容(暫停先賣後買)出現在多方、
被空方閘門擋掉;空方清單零處置零暫停;`/api` 在本機 404 時頁面正確降級不報錯。

### ③ 部位標記 UI + 同步 API(`api/daytrade.py`)

**沒有部位資料,強制回補提醒就完全沒東西可提醒** —— 而那是整個當沖層裡價值最高、
最不受延遲影響的一層。盯盤跑在 GitHub Actions 上讀不到 localStorage,所以必須上雲端。

沿用 `api/watchlist.py` 已驗證過的模式(GitHub Contents API + 共用 `WATCHLIST_SECRET`,
沒設 secret 一律拒寫)。兩個端點:
- `?what=prefs` / POST `{prefs}` → `config/daytrade_prefs.json`(部分更新)
- `?what=positions&day=` / POST `{positions, day}` → `data/daytrade/positions-YYYY-MM-DD.json`

**隱私界線(白名單制)**:
- prefs 只收 `_PREF_NUM`/`_PREF_BOOL`/`_PREF_LIST` 列出的欄位,**含 `quota_twd`**
  (方案 B,只放寬到「額度上限」一個數字)
- positions 只收代號/方向/進場價/張數/停損/開倉時間/是否平倉 ——
  **`total_pnl`、`account_balance` 這類欄位就算前端送上來也會被丟掉**
- 防呆:NaN、負數、超範圍、亂碼代號、超量(50 筆上限)、`day` 路徑注入全擋

前端在當沖分頁頂部加「我的當沖部位」區塊(新增/平倉/重開),
未平倉時顯示警告框並標出有幾筆是先賣後買(講明券差→違約交割的後果)。

### 測試

新增 `tests/test_daytrade_api.py`(11 條),重點全在「不該收的有沒有擋掉」——
這是**公開端點而且會寫進 repo**。目前 **7 個測試檔全部通過**。

### 現在的完整狀態

| 層 | 狀態 |
|---|---|
| 標的池(多空兩欄 + 六道閘門 + 成本帶) | ✅ 已上線,生產跑過 |
| 價位穿越推播 + 分級 | ✅ 已實作,**待延遲數據驗證是否值得** |
| 持倉風控 + 強制回補 | ✅ 已實作,**待你登記部位才會作用** |
| 當沖台帳(含未推播對照組) | ✅ 已實作,累積中 |
| 網頁分頁 | ✅ 已接,瀏覽器實測過 |
| 偏好/部位雲端同步 | ✅ API + UI 完成 |

### 待辦(需要你或需要時間)

1. **Vercel 環境變數**:`api/daytrade.py` 與 watchlist 共用 `WATCHLIST_SECRET` /
   `GITHUB_TOKEN` / `GITHUB_REPO`,應該已經設好;可用 `/api/daytrade?check=1` 確認。
   ⚠️ Vercel 環境變數**要 Redeploy 才生效**。
2. **9/09 開盤後看延遲數據** → 決定第 2 層留、調、還是降級。
3. **累積一個月後看 `--summary`** 的 pushed vs not_pushed —— 若兩組沒差,
   要調的是分級邏輯而不是推更多。

---

## 66. 當沖層事故與大改:排程從沒觸發 + 從「清單」變「可執行建議」(2026-09-09)

### 🔴 事故:daytrade.yml 從來沒有被排程觸發過

使用者 9/09 早上反映「為什麼沒有即時推薦標的」。查下去發現**根因不是設計,是它沒跑**:

- `gh run list --workflow=daytrade.yml` 只有 9/08 我手動觸發的兩次
- 9/09 沒有 pool、沒有 gates、沒有延遲量測
- 網頁整天顯示 **9/08 的標的池**,而且**只在頁尾小字標日期,沒有任何過期警告**

**根因:我把當沖層架在 GitHub 內建 `schedule:` 上,而本專案早就知道那條路不可靠。**
HANDOFF 第 1 節寫著「GitHub 內建 schedule 已移除(會延遲)」,daily/premarket 都改走
外部 cron(cron-job.org 以 `workflow_dispatch` 觸發),實測每天準時。我沒沿用。

**修法**:改掛 `workflow_run: workflows: ["Premarket Watch"]`。那支是外部 cron 觸發的,
08:45 台北跑完後接著建池 + 進盯盤(盯盤本來就會等到 09:00 才開始掃),時序剛好。

⚠️ Premarket 一天跑兩班(08:45 / 09:25),兩班都會觸發。所以:
- **刻意不設 concurrency group** —— 設了 `cancel-in-progress:false` 第二班會排隊等
  第一班的 4.5 小時盯盤跑完;設 `true` 又會把正在盯盤的砍掉、連當日去重狀態一起丟。
- 改用檔案守衛:`今日 pool 已存在 且 已過 09:05` → 直接 exit 0。
  池子不存在才補做 —— 這同時是自我修復路徑(第一班若在建池前掛掉,第二班會補上)。

### 🔴 網頁沒有過期警告(鐵則二違規)

`loadAlerts` 早就有 `d.date >= today` 的保護,我沒把同一條套到當沖分頁。
已補:日期落後就在最上方顯示紅框「**這是 X 的標的池,不是今天的**,
下方所有價格與計畫都以 X 的收盤為基準,不要直接照著下單」。

### 從「候選清單」變成「可執行建議」(`scripts/daytrade/plan.py`)

使用者要的是**進場價/出場價/停損價/預計獲利/預計成本/為什麼推薦/線圖/指標/參考資料**。
第一版的標的池只回答「今天哪些檔可以做」,少了這些就只是 watchlist。

新增 `plan.py`,四條設計原則:
1. **停損先決定**,其他從它推出來(當沖唯一硬約束是當天一定要平掉,風險必須先框死)
2. **預計獲利一律扣成本才顯示** —— 毛利沒有意義,來回成本吃掉 0.5%~1.3%
3. **算不出來就說沒有** —— ATR 缺/價格 0/停損小於一個 tick → 回 None,不給猜的數字
4. **目標價要被壓力支撐修正** —— 純 1.5R 推出來的目標可能落在前高上方一大截

⚠️ **兩個實測後才發現的參數問題**:
- `atr_stop_mult` 原本用 1.0,實測台半停損距離 **5.56%**、最大虧損 **10,766 元 =
  25 萬額度的 4.3%**,而目標要走 8.28% 盤中根本不會到。ATR(14) 是**整天**的區間,
  當沖只持有幾小時 → 改 **0.4**,停損落在 2.27%、目標 3.4%。
- `config` 的 `max_risk_pct_of_quota: 2.0` **根本沒被用到**,部位大小只看「買得起幾張」。
  已改成「買得起」與「風險預算允許」取小的 → 最大虧損壓到額度的 1.99%。

**所有價格都對齊 tick**(`align_to_tick`):卡片上出現 74.37 這種掛不進去的價格,
使用者得自己換算,而當沖沒有這個時間。進場價保守取整(多方往上、空方往下)——
寧可算出來的預期獲利略低,也不要用搶不到的價位算出漂亮數字。

### 卡片式呈現(網頁 + Discord)

網頁 `dtPlanCard()`:四格價位(進場/目標/停損/張數)+ 損益列(預計獲利已扣成本/
最大虧損/預計成本/風報比)+ 推薦理由 chips + 指標 chips + 參考連結
(CMoney 線圖 / Yahoo 報價 / 公開資訊)+「＋ 登記這筆」一鍵帶入。
預設只顯示各方向 6 張,可展開。

Discord:
- **新增盤前推播** `push_premarket_picks()` —— 第一版只在盤中觸發時才推,
  **開盤前完全收不到任何東西**,而當沖最需要準備的就是開盤前。
- 盤中訊號卡升級成與盤前同樣的結構(原本只有停損,沒有目標價與預期獲利)。
  盤中會**用觸發當下的即時價重算計畫** —— 盤前那份以昨收為基準,價格已經動了。

### 部位管理改寫

使用者反映「新增部位要更簡單更輕鬆,也要能刪改」。第一版是**連續 5 個 `prompt()`**,
中途按一次取消就全部白打,而且只能切換平倉、不能改也不能刪。

改成內嵌表單(`.dt-form`):一次填完、可編輯(`改`)、可刪除(`刪`,有確認)、
可平倉/重開。標的池卡片的「＋ 登記這筆」會把代號/方向/進場/停損/張數**全部預填**。
部位列新增**現價與未實現損益**(走既有 `LIVE` 報價快取,並把未平倉代號加進
`liveIds()` 讓它跟著即時輪詢更新),停損被觸及會標紅。

⚠️ 踩到的坑:我一開始寫 `QUOTES[p.stock_id]` —— **那個全域根本不存在**(我憑印象編的)。
實際的報價快取叫 `LIVE`,由 `liveQuotes()` 填。查了才知道。

### 測試

新增 `tests/test_daytrade_plan.py`(19 條)。其中一條是把使用者的需求清單直接釘進測試:
`test_plan_embed_has_every_field_the_user_asked_for` 逐項檢查卡片有沒有
進場/目標/停損/預計獲利/最大虧損/預計成本/風報比/為什麼推薦/指標/參考 ——
少一個就紅。**8 個測試檔全部通過。**
...
## 68. 實戰卡片抓到三個錯:做多目標低於進場、昨收造成假穿越、漲停還推做多(2026-09-09)

使用者貼出 Discord 實際收到的卡片,上面有明顯錯誤。逐一查證後全部重現並修正。

### 🔴 錯誤一:做多的目標價低於進場價

卡片顯示台塑化(6505)做多:**進場 87.5、目標 83、停損 85.8**,
而且標成「+5.14%、預計獲利 +7,976 元、風報比 2.65」——
目標比進場低 5%,那是**虧的方向**,數字卻全是正的。

**根因**:`build_plan` 的壓力修正只檢查 `0 < resistance < target_raw`,
**漏了 `resistance > entry`**:

```python
if side == "long" and resistance and 0 < resistance < target_raw:   # ← 錯
elif side == "short" and support and target_raw < support < entry:  # ← 對(有 < entry)
```

**不對稱的疏漏** —— 做空那條本來就檢查了 `support < entry`,做多那條沒有。
台塑化當天大漲 9.86%、早就站上 20 日高(83.1),所以 `resistance(83.1) < entry(87.5)`,
目標被壓到 83。

**為什麼數字看起來正常**:距離一律用 `abs()` 算,方向錯了 `target_pct` / `rr` /
`gross_profit` 全都還是正數 —— 錯得很「漂亮」,這才是最危險的地方。

**修法**:
1. 做多改成 `entry < resistance < target_raw`
2. **加不變式安全網**:做多必須 `stop < entry < target`、做空必須 `target < entry < stop`,
   違反就回 `None`。這不是重複檢查 —— 上面任何一段邏輯寫錯都會在這裡被擋下來,
   而不是產生一組看起來合理、實際上是虧的價格。

### 🔴 錯誤二:「觸發 整數關卡 85」但即時價 87.45

85 這條水位在上午就被穿越過了,為什麼 13:17 才推?

**根因**:`get_quotes` 的最後一層降級是**本機昨收**(`source="close"`)。
台塑化昨收 **75.8**、盤中 **87.45**。只要有一輪 MIS 失敗拿到昨收,
`prev_price` 就變成 75.8,下一輪拿到真實價時就「穿越」了
**75.8~87.45 之間的每一條水位** —— 85 正好在中間。

**修法**:`scan_once` 直接濾掉 `source == "close"` 的報價,整輪回
`no_live_quotes`。昨天的價格不是即時價,不能拿來做穿越判斷。

### 🟠 錯誤三:已漲 9.86%(近漲停)還推做多

台股上下限 ±10%,漲停買不到、跌停賣不掉,而且隔天跳空風險完全不對稱。
既有的 `intraday_scan` 有 `MAX_CHG = 8.0` 的同款保護,**當沖層第一版漏了**
(分數雖然有扣位階分,但 66 分仍高於 `min_score: 55` 就推出去了)。
已加 `MAX_CHG_PCT = 8.0` 硬擋,多空對稱。

### 🟡 順帶修:整數關卡容差 3% → 1%

離現價 2.8% 的整數關卡早就在幾小時前穿越過了,留著只會在報價抖動時製造假訊號。
收到 1%(約當一根 5 分K 的幅度)。
...
- 「站上 5 日與月線(多頭排列)」 ← 與上面重複
- 「**RSI 62(中性)**」 ← 與上面的「偏強」**互相矛盾**(兩處用不同門檻)
- 「波動是來回成本的 7.9 倍」 ← 與訊號層的「空間 7.9x 成本」重複

均線與 RSI 的判讀統一交給 `direction_bias` 講(那裡才有完整投票結果);
`build_reasons` 只補它沒講的(波動幅度、量能、閘門)。「空間」留在標的池那邊
(盤前卡只有池子的理由,拿掉盤前就看不到操作空間),訊號層那句較短的重複版移除。

### 測試(7 條新的,而且驗證過真的抓得到)

`test_daytrade_plan.py` +4:壓力低於進場不可當目標、壓力在區間內仍要正常修正、
做空的對稱情況、**不變式掃描**(48 組參數組合驗證價格順序恆成立)。
`test_daytrade_engine.py` +3:昨收報價不可造成假穿越、漲停不推多、跌停不推空。

**驗證測試真的有效**:分別還原「壓力方向檢查」與「昨收過濾」兩個修正,
對應的測試各自變紅,還原修正後再度全綠。

⚠️ **踩到的坑**:我第一次是用 `>>` 把新測試**附加在檔案末尾**,
而那些檔案的 runner 是 `if __name__ == "__main__":` 迭代 `globals()` ——
Python 由上而下執行,**runner 跑的時候那些函式還不存在,所以完全沒被執行**
(輸出一片綠但實際上少跑了 7 條)。已搬到 runner 之前。
**日後往這類自製 runner 的測試檔加東西,一定要插在 runner 之前。**

### 全池驗證

重建標的池後掃描全部 119 檔的計畫:**價格順序全部正確**。

---

## §69 當沖層逐檔排錯 + 多週期支撐壓力（2026-09-10）

使用者：「檢查今天的當沖系統，且暫時先停止今天的即時通知，我覺得目前的不夠準確
而且訊號過多，策略要加入看日線、小時線、月線支撐壓力，且目前的當沖有很多問題，
逐檔檢查並排錯。」

**通知已停**：兩個重複的 watch job 都取消、`Day Trade (independent)` workflow
設為 `disabled_manually`。`Intraday Signal Scan`（既有選股層）維持啟用，未受影響。
**要重新開啟必須等使用者說。**

### 一、`data/levels.parquet` 停更 16 天（最嚴重）

`intraday_scan._git_publish` 的 `_all` 清單有 `docs/levels.json` 卻**沒有 parquet**，
所以水位檔從 8/25 起就沒再進 repo。標的池的價格因此錯 12~21%
（2426 鼎元：檔案 82.2、實際 99.5）。已補進清單並立即重建。

> 這是本專案第三次踩「產物只在 job 最後才 commit / 根本沒列進 commit 清單」。
> 每次的症狀都是「看起來有在跑，但資料是舊的」。

### 二、`risk_quota` 與 `quota` 被混為一談

`_plan_fields` 把「每筆可用資金」（總額度 × 50%）當成 `quota` 傳進 `build_plan`，
而 `build_plan` 內部拿 `quota × max_risk_pct` 當單筆風險預算 ——
於是風險預算變成真實額度的 **1%** 而不是設定的 2%，**靜默地**把 43 檔的計畫算成 `None`。
已把「資金上限」與「風險基準」分成兩個參數（`quota` / `risk_quota`），兩條路徑都改。

### 三、昨日高低拿到的是**今日盤中**高低（訊號不準的主因）

`_levels_for` 傳的是 `quote.high` / `quote.low`，而 `Quote.high/low` 是
MIS 的 `h`/`l` ——**今天的**最高最低，不是昨天的。後果有兩層：

1. 卡片標「昨日高點」卻顯示今天的高點，資訊直接是錯的；
2. **這條水位在功能上是壞的**。今日高點依定義 ≥ 現價且隨現價移動，
   而 `detect_cross` 要 `price >= level × 1.002` 才算上穿 —— 永遠不可能成立，
   除非報價的 `high` 欄位落後 `price` 欄位超過 0.2%。
   也就是說它**只會在資料不同步時觸發，那全部是假訊號**。空方的今日低點同理。

而 `prev_high`/`prev_low` 權重 0.85 是第二高的 —— 等於把第二重要的水位
換成了雜訊產生器。改成取 `tech_snapshot` 從本機日線算好的
`indicators.prev_high/prev_low`（昨日那根 K 棒，盤前算好、盤中不變）。

### 四、盤中計畫路徑繞過盤前的四道防護

`scan_once` 裡的 `P.build_plan` 呼叫跟 `universe._plan_fields` 早就分岔了：

| 防護 | 盤前 | 盤中（修正前） |
|---|---|---|
| 壓力/支撐 | 多週期匯流區 | **20 日高低** |
| 風報比下限 `min_rr` | 有 | **無**（RR 0.66 照推） |
| `atr_stop_mult` / `rr_target` | 讀 config | **寫死預設** |
| `risk_quota` 分離 | 有 | 無 |

20 日高在突破日會落在現價**下方**（2026-09-09 台塑化就是這樣把做多目標算到進場之下），
不變式雖然會擋，但是**靜默地**把計畫變成 `None`。兩邊已對齊，並新增 `_zone_bounds`
依**即時價**重挑壓力/支撐（盤前那組 `mtf_resistance` 是以昨收為基準，跳空後就不對了）。

> **「同一件事有兩條路徑」是這個專案反覆出事的地方。改一邊就要同時改另一邊。**

### 五、停損距離整條在雜訊帶裡（低波動金融股）

兆豐金 / 彰銀 / 元大金 / 永豐金：ATR 小 × `atr_stop_mult` 0.4 → 停損只有
0.92~1.12%，而來回成本 0.55~0.72%，**倍數只有 1.38~1.68**。
被雜訊掃掉時虧的錢跟手續費同一個量級，等於在付費擲硬幣。

它們能通過 `min_edge_ratio: 3.0` 是因為成本也低 —— **比值不能取代絕對距離**。
新增 `risk.min_stop_cost_mult: 2.0`，停損距離取
max(ATR×倍數, 2 個 tick, 來回成本×2.0)。修正後最小值 1.38× → **2.03×**。

### 六、多週期支撐壓力（使用者要求的新策略）

新檔 `scripts/daytrade/levels_mtf.py`。月線（日線重採樣，零 API）+
日線（擺動高低點 + MA20/60/120，零 API）+ 小時線（yfinance 60m，要網路）。
價位差 0.4% 以內合併成一個 zone，權重相加，`confluence` = 有幾個週期背書。

新水位權重：`mtf3: 1.10`（比開盤區間還硬）/ `mtf2: 0.90` / `mtf1: 0.60`。

**四道濾網**（少一道就會退回「訊號多而不準」）：

1. **只留今天走得到的**：範圍 max(6%, 2×日 ATR)。第一版用 5×ATR，
   對 ATR 9.5% 的鼎元算出 ±47%，等於整整一週的幅度，把幾年前的價位放回來了。
2. **只收兩個週期以上背書的**（`min_mtf_confluence: 2`）。單週期水位權重 0.6
   比 VWAP 還低，全收會佔新增水位的 38%（67/175）—— 用最弱的水位把訊號量灌回去。
3. **依離現價的距離重排再取前 6 個**。⚠️ 標的池存的是 `MTF.summary()` 的輸出，
   那是依**權重**排的；直接 `[:6]` 會留下「很重但今天走不到」的、砍掉近的
   （6226 光鼎現價 29.65：24.50 排第 7、27.12 排第 8，取前 6 會保留前者丟掉後者）。
4. **貼著現價或貼著別條水位的不要**（至少 0.35%）。前者第一個 tick 就「突破」；
   後者兩條線相距不到一次來回成本，交易上是同一筆，會雙重觸發。

**小時線的接法**：`tech_snapshot` 是對約 200 檔跑的，那裡開小時線 = 200 次
yfinance 呼叫，所以寫死 `with_hourly=False` —— 結果**小時線從頭到尾沒真的接上**，
`mtf3` 形同死碼。新增 `engine._enrich_hourly()`，在池子建好之後只對入選的
幾十檔補算（盤前不趕時間）。實測 33 檔約 6 秒，匯流度分佈
1:92 / 2:91 / 3:63 —— `mtf3` 這才真的活起來。

### 七、整數關卡關閉

`use_round_numbers: false`。它本來就不是真的支撐壓力（心理價位，常穿了又回），
權重也是全部裡最低的；在只有「當天水位」的年代它是拿來湊數的，
現在有了真的多週期支撐壓力，它的角色已經被取代。

### 八、重複盯盤的檔案守衛從來沒生效過

守衛是在 checkout 出來的 repo 裡找 `data/daytrade/pool-$TODAY.json`，
而原本 commit 只在 job 的**最後一步** —— 盯盤要跑 4.5 小時，
09:25 那一班永遠找不到那個檔案 → **兩個盯盤迴圈同時跑、Discord 雙份通知**
（2026-09-10 實際發生，兩班都手動砍掉）。

抽出 `.github/scripts/daytrade-publish.sh`，**建完池馬上發布**再進盯盤。
順帶解決另一件事：網頁與盤前卡片本來也要等到 13:35 才更新。
最後那一步也改成 `if: always()`（盯盤中途掛掉時台帳才不會整天丟失）。

### 效果

| 指標 | 修正前 | 修正後 |
|---|---|---|
| 標的池 | 多 56 / 空 27（有計畫僅 34） | 多 26 / 空 7（**全部都有計畫**） |
| 每檔水位數 | 10.8 | 8.5 |
| 平均水位品質權重 | 0.834 | **0.924** |
| 停損 ÷ 來回成本（最小） | 1.38× | **2.03×** |
| 小時線水位 | **0 條** | 316 條 |
| 匯流度 3 的水位 | **0 個** | 63 個 |

「沒有可執行計畫的不進池」也一併加上（`stats["no_plan_dropped"]`）——
列在清單上但算不出進場/停損/目標的標的對當沖沒有用處，只是佔版面。

### 顯示

Discord 卡片與網頁卡片都新增「多週期支撐壓力」一列（離現價最近的 4 個，
標示壓力/支撐、幅度、哪些週期背書）。**算了不顯示等於沒做。**

### 測試

`tests/test_daytrade_levels_mtf.py` **新檔 10 條**（合併/排序/相關範圍/
最遠水位剔除/nearest 不回現價下方的壓力/資料不足回空）。
`test_daytrade_signals.py` +5（距離重排、貼現價剔除、貼別條剔除、匯流度閘門、壞欄位不炸）。
`test_daytrade_plan.py` +4（`risk_quota` 分離、預設回退、停損成本下限含空方對稱、
ATR 夠寬時下限不覆蓋）。
`test_daytrade_engine.py` +2（昨日高低來源、`_zone_bounds` 只認匯流水位）。
**9 個測試檔全綠。**

⚠️ 舊測試 `test_upward_cross_fires_long_signal` 的 docstring 原本寫
「用 high 當水位」—— **測試把 bug 寫進去了**。修 bug 時要一起看測試在斷言什麼。
