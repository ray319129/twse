...
短線選股(隔日沖 / 隔週 / 月內)+ 自選股監控 + Email 通知,跑在 GitHub Actions,**完全免費**。

> ⚠️ **本系統提供的是「依信心分排序的股票清單」,不是投資建議。** 分數高 ≠ 該買。

## 它做什麼

- 每個交易日 16:30 (台北時間),GitHub Actions 自動:
  1. 抓全市場(上市 + 上櫃約 1900 檔)當日資料、算技術指標
  2. 用**免費資料**(價格 + TWSE 估值快照)對全市場算 0~100 **信心分**並排序
     (趨勢健康 + 相對強度 + 短線時機/量能 + 品質估值 + 流動性,並重罰「漲過頭」)
  3. 只取最高分的 **核心 10 檔**(今日有新鮮觸發、可考慮進場)+ **觀察 20 檔**(醞釀中)
  4. 只對入榜的核心 + 自選池**才補抓 FinMind 籌碼/財報**(控制 API 額度,避免打爆免費版)
  5. 寄 Email:核心精選 + 觀察清單 + 產業熱度 + 自選池
